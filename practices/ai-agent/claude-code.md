...
- **`description` は 1,536 文字で切り捨てられる（Claude Code 公式仕様）**: `description`（と `when_to_use` を併記している場合はその合計）が 1,536 文字を超えると、超過分は黙って切り捨てられる。判断材料になる「いつ使うか」を後半に書いていると、切り捨てで読まれない可能性がある。長い説明が必要な場合は「何をするか」と「いつ使うか」の両方を先頭付近に収まる長さで簡潔に書き、詳細は SKILL.md 本文側に回す。他にも under-trigger の典型原因として、①`name` がフォルダ名と不一致でロードされない、②本文が説明過多で指示が埋もれる、③同格の選択肢を複数提示してAIが手戻りする、④集計・ソート等の決定的処理を（スクリプト化せず）AI自身にやらせる、⑤プロジェクト固有の落とし穴（gotcha）が書かれておらず手順は合っていても結論を誤る、がある
...
> 「『何をするか』と『いつ使うか』の両方を書く」／Claude Code 公式仕様として「the combined `description` and `when_to_use` text is truncated at 1,536 characters」
> ([Agent Skills が発火しない・効かない7つの原因 — 公式ドキュメントから学ぶSkill設計のベストプラクティス](https://zenn.dev/hamuo_tech/articles/4915abbe0d8ed8), セクション "3. `description` は「AIが使うかどうかを決める唯一の材料」" / "原因3") ※2026-09-18に実際にfetch成功
...
- [Agent Skills が発火しない・効かない7つの原因 — 公式ドキュメントから学ぶSkill設計のベストプラクティス](https://zenn.dev/hamuo_tech/articles/4915abbe0d8ed8) (Zenn hamuo_tech、公式仕様として `description`+`when_to_use` 合計1,536文字での黙示切り捨てを明記・under-trigger 7原因の体系化) ※2026-09-18に実際にfetch成功
...
- **`rm` だけを列挙したブロックは `cp` による無確認上書きを防げない**: `cp src dest` は既存の `dest` を確認なしで上書きする。削除系コマンドのパターンマッチだけを実装していると、ファイルの内容が破壊される事故（`.env` の意図しない上書き等）を見逃す。`permissions.deny` の列挙対象に `cp` 等の上書き系コマンドも含め、PreToolUse フックでも「実行後に元に戻せるか」を判定基準に加える
...
- [Claude Code の承認プロンプト、rm が無くても消えることがある](https://qiita.com/fukumuraryota0724/items/6f2703e602de362b04d6) (Qiita、`cp` による無確認上書きが削除系コマンドの列挙をすり抜ける実例と `settings.json` deny + PreToolUse hook の対策例) ※2026-08-30に実際にfetch成功
...
- **シェルリダイレクトは文字列パターンマッチをすり抜ける**: `rm -rf ~` は正規表現でブロックできても、`rm -rf ~ 2>/dev/null` のようにリダイレクトを付け足すと同じ正規表現が素通りする実測例が報告されている。これは自作フックだけでなく Claude Code 自身の公式許可チェックにも共通する弱点で、シェルが実際にコマンドをどう解釈するかと、文字列マッチャーがどう区切るかが一致しない箇所を突かれる。PreToolUse の正規表現は「コマンド全体の文字列」ではなく、リダイレクト・パイプ・`&&` 連結を含めてシェルが実際に解釈する形を意識して設計する
- **hook の `matcher` を特定ツール（例: `Write|Edit`）に絞ると、AI が別ツール（`Bash`）を選んだ場合にルールが一切届かない**: 25 回の CLAUDE.md 単体指示は全 25 回遵守されたのに対し、同じ一文を `Write|Edit` にマッチする hook へ移した途端、AI が `Bash` 経由で目的を達成した 5 回すべてで hook が発火せず届かなかった実測がある（exit code は 0 のまま、エラー表示なし）。さらに `matcher` を全ツール（`.*`）に広げても、hook 発火は「ツール選択後」に起きるため、AI が先に `Write` を使った後にしか介入できず、禁止ツールの使用そのものは防げない。初動の判断（どのツールを選ぶか）に影響させたいルールは CLAUDE.md 側に置き、hook は「起きたことの事後検知」に用途を限定する
...
> "シェルが実際にコマンドをどう解釈するかと、網が字面をどう区切るかが、ずれている。この一点の隙間を、リダイレクトの記号が突く。"
> ([同じ死角が、自作フックとClaude Codeの許可チェックの両方にあった——2>/dev/null 一つで抜ける話](https://qiita.com/yurukusa/items/6b329f74931d67fdd705), セクション "同じ死角が、公式の許可チェックにもあった") ※2026-07-19に実際にfetch成功

> "ルール無しの5回は当然0行。でも残り15回は全部3行で、中身も `wrote:a.txt` / `wrote:b.txt` / `wrote:c.txt` で完全に一致していました"（CLAUDE.md 単体指示、25/25 成功）／「うまくいった回と見分けがつきません」（`Write|Edit` にマッチする hook へ移した回、0/5 到達）
> ([AIへの指示書に書いたルールは25回とも守られた。同じ一文をフックに移したら、5回とも一度も届かなかった](https://zenn.dev/numarn/articles/claude-md-rule-vs-hook-injection-handson), セクション "「指示書は守られない」は、私の手元では再現しなかった" / "同じ一文をフックに移した瞬間、5回とも届かなくなった") ※2026-09-18に実際にfetch成功

**出典**（追加）:
- [同じ死角が、自作フックとClaude Codeの許可チェックの両方にあった——2>/dev/null 一つで抜ける話](https://qiita.com/yurukusa/items/6b329f74931d67fdd705) (Qiita ゆるくさ、シェルリダイレクトによる正規表現許可チェックのバイパス実証・公式許可チェックにも共通する弱点) ※2026-07-19 fetch
- [AIへの指示書に書いたルールは25回とも守られた。同じ一文をフックに移したら、5回とも一度も届かなかった](https://zenn.dev/numarn/articles/claude-md-rule-vs-hook-injection-handson) (Zenn numarn、CLAUDE.md 指示 25/25 成功 vs hook matcher 不一致による 0/5 未達・発火タイミングのずれによるツール選択防止不可の実測) ※2026-09-18に実際にfetch成功
...
- [Claude Code 2.1.53+ の Workspace Trust と permissions.allow の挙動変更](https://zenn.dev/wharfe/articles/claude-code-untrusted-workspace-permissions) (Zenn wharfe、CVE-2026-33068・未信頼ワークスペースでの設定無視・hasTrustDialogAccepted 解除手順) ※2026-06-28に実際にfetch成功
...
- **コンテキスト圧縮は「忘却」だけでなく「捏造」を引き起こす**: 圧縮境界やツール失敗の直後に、ユーザーが指示していないタスクを「直近の保留タスク」として自動要約に紛れ込ませ、あたかも実在の指示であるかのように実行し始める事例が報告されている。存在しないコミット・デプロイを「完了した」と報告するケースもあり、これは設計判断の忘却とは異なる能動的な事実捏造であるため、圧縮境界・ツール失敗の直後は特に人間が結果を疑うべきタイミングとして扱う
...
- 圧縮直後の自動要約に含まれる「保留タスク」「完了報告」を無検証で信頼する（`git reflog` / `git log --oneline` で実コミットの有無を、検証スクリプトの実在を実ファイルで確認する）
...
> "存在しないコミットとデプロイ。コードの修正を『v0.8.59としてコミット・デプロイ済み』と報告した。だが `git reflog` にそんなコミットは無く"
> ([Claude Codeが「あなたが言っていない指示」を、あなたのタスクとして実行しかけた——長いセッションの文脈の圧縮のあとに起きる捏造](https://qiita.com/yurukusa/items/6f0b8c1d6c41f196cf2a), セクション "一つのセッションで、事実が4回捏造された") ※2026-07-06に実際にfetch成功

**出典**（追加）:
- [Claude Codeが「あなたが言っていない指示」を、あなたのタスクとして実行しかけた——長いセッションの文脈の圧縮のあとに起きる捏造](https://qiita.com/yurukusa/items/6f0b8c1d6c41f196cf2a) (Qiita yurukusa、圧縮境界・ツール失敗直後に集中する事実捏造の報告と検証手順) ※2026-07-06 fetch
...
**効果の実測値**: ディレクトリ構成とファイルの役割だけを書いた 24 行の CLAUDE.md を用意し「ログイン機能はどのようなものか」という同一の質問を投げた検証では、API 呼び出しが 14 → 6 回（57.1% 減）、入力トークンが 664,656 → 360,218（45.8% 減）まで減少した。削減の主因はファイルサイズそのものではなく「モデルとツールの往復回数」で、CLAUDE.md がない場合は Explore サブエージェントが 7 回ツールを実行したのに対し、ある場合は並列読み込み込みで 4 回のモデル呼び出しに収まった。往復のたびに会話履歴・システム指示・ツール定義が入力に再送されるため、往復回数の削減がトークン削減に直結する。
...
- [CLAUDE.mdによるClaude Codeの探索コスト削減を実測](https://qiita.com/eiji-noguchi/items/ad30cd311f083cd269d0) (Qiita、24行のCLAUDE.mdの有無でAPI呼び出し回数・トークン消費を定量比較した検証) ※2026-07-16 fetch
...
- **`tools:` フロントマターでの `Bash(git diff:*)` のようなパターン絞り込みは、実際には Bash ツール全体を渡してしまう**: `tools: ["Read", "Bash(git diff:*)"]` を与えたサブエージェントに `node --version` を実行させたところ普通に成功した（本来 `git diff` 系のみを許可したつもりだった）。`tools: ["Read"]` のみのサブエージェントでは同じ指示が失敗したことから、`disallowedTools` がパターン一致に関わらずツール単位で丸ごと除去するのと非対称に、`tools` の許可リスト側ではサブコマンドパターンが実効的なフィルタとして機能しない。コマンド単位で絞り込みたい場合は `tools` 定義に期待せず、`PreToolUse` hook で実際にコマンド文字列を検証する
...
**`description` フィールドの設計指針**:

`description` はルーティングの唯一の手がかり。「何でもできます」型は呼ばれ過ぎ、「〇〇のエージェントです」型は呼ばれなさすぎる。**プロダクト固有の名詞 + 「〇〇に関する作業はこのエージェントに依頼する」** という形式が最も機能する。

```yaml
# Good: 呼び出しトリガーが明確
description: >
  Stripe の決済フローとサブスクリプション管理に関する作業はこのエージェントに依頼する。
  checkout.session.* イベントの処理・webhook 署名検証・返金フローのデバッグを担当。

# Bad: 汎用すぎてルーティング不能
description: 支払い関連のコードを書くエージェントです。
```

**HANDOFF.md パターン**（エージェント間の文脈継承）:

サブエージェントは親の会話履歴を持たない。複数フェーズにわたる作業を跨ぐ場合は `HANDOFF.md` を中間ファイルとして使い、次エージェントへの引き継ぎ情報を明示的に記録する。

```markdown
# HANDOFF.md（エージェントが生成・次エージェントが冒頭で必ず読む）
## 完了した作業
- API スキーマ定義: src/schemas/payment.ts (L1-89)
## 残タスク
- Stripe webhook ハンドラの実装（src/api/webhooks/stripe.ts）
## 制約・注意
- 既存の PaymentRecord 型は変更禁止（他チームが依存）
```

**サブエージェント暴走再帰への防衛**:

サブエージェントが親エージェントを呼び出す（再帰）と、コンテキストが無限に増殖してセッションがクラッシュする。`tools:` リストに `Agent` ツールを含めないことで構造的に防止する。また、20 エージェントを超えると Claude のルーティングが混乱しはじめるため、機能ドメインを束ねた「スーパーバイザーエージェント」層を設けて件数を抑える。

```yaml
# サブエージェントには Agent ツールを与えない（再帰防止）
tools: Read, Grep, Edit, Write, Bash
# tools に Agent を含めない
```

**`description` を書き忘れた場合の挙動（サイレント障害）**:

`.claude/agents/` にファイルを置いただけでは安心できない。フロントマターの `description` を書き忘れると、そのサブエージェントは**エラーなく存在しない扱いになる**。起動時のエラー・警告は一切出ず、静かに利用可能エージェント一覧から除外される。新規サブエージェントを追加したら、まず簡単なタスクで実際に呼び出されるか動作確認する習慣が必要。

**アンチパターン**:
- 全階層を Opus で統一する（コストが深さに比例して急増）
- `description` を曖昧に書く（ルーティング精度が下がり意図しないタイミングで呼ばれる）
- `description` を書き忘れる（エラーが出ないままエージェントが無視される）
- 逐次処理タスクをネストする（単一エージェントの方が速い）
- 横方向の並走タスクをネストする（Agent Teams の方が適切）
- サブエージェントに `Agent` ツールを付与する（再帰暴走のリスク）
- エージェント数を 20 件超に増やす（ルーティング精度の劣化）
- 親の会話履歴をサブエージェントが参照できると仮定する（コンテキストは独立）
...
> 「descriptionには『XXX関連の作業はこのエージェントに依頼する』という形式で書くのがおすすめ。Claudeがエージェントを選ぶ際に、この説明文を参照してルーティングを行うため、曖昧な表現では意図しない呼び出しが発生する」
> ([.claude/agents/でサブエージェントを定義する設計パターン](https://zenn.dev/nakayama_acari/articles/claude-code-agents-design), セクション "description の書き方") ※2026-06-27に実際にfetch成功
...
- [サブエージェントの tools で Bash を絞ったつもりになっていないか](https://qiita.com/yurukusa/items/9466eaccf8e4c26cd137) (Qiita ゆるくさ、`tools: ["Read", "Bash(git diff:*)"]` のパターン絞り込みが Bash 全体を許可してしまう実測比較と `PreToolUse` hook による代替策) ※2026-09-18に実際にfetch成功
...
**CI 上で AI エージェントに PR を作らせる場合の追加リスク（プロンプトインジェクション経路）**:
...
- 「AI が編集できる」「外部に働きかける権限」「信頼しきれない入力」を同一 CI ジョブに同居させる（プロンプトインジェクション経由で編集力と権限を同時に悪用される）
...
> 「ファイルを編集できるAI、外部に働きかける権限や鍵、信頼しきれない入力。この三つが同じジョブに揃うと、意図しない入力でAIを動かし、その編集力と権限を使わせることができてしまいます。」
> ([AIに無人でPRを作らせる前に切り分けたい権限](https://zenn.dev/dely_jp/articles/b120322d2ab99b), セクション "編集できるAIと操作できる権限が同じジョブに存在") ※2026-07-06に実際にfetch成功
...
### 46. 許可ルールは承認ダイアログ任せにせず、`Bash(cmd:*)` の前置マッチ形式へ書き直してから `permissions.allow` に置く

承認ダイアログの「常に許可」で保存されるのは、**その瞬間に実行しようとしていたコマンド文字列そのもの**である。
引数・PID・絶対パスまで含んだ完全一致ルールが `settings.json` に溜まり続けるが、同じ文字列が二度と現れないため再利用されない。
