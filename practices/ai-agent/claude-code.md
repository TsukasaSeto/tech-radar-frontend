...
**チーム規模のメモリ共有では、キュレーションより検索基盤への投資が効く場合がある**: 本ルール前段の `MEMORY.md` インデックス方式は個人〜小チーム規模を想定しているが、組織横断で数千件規模の会話ログを共有する場合は前提が変わる。ある事例では、Claude Code の会話ログを Git 管理の Markdown（正本）+ SQLite（全文検索・意味検索の二重索引）で約7,000件蓄積し、「完璧に整理する」代わりに「何でも残して検索を強くする」方針を取っている。検索は2段階（候補を10件程度に絞り込み→Claude が意図に応じて2〜3件を選択）で、フロントマターのリンクを辿って関連メモリも自動的に手繰る設計にしている:

> "人の頭の中にしかなかった細かい文脈が、チームの記憶として永続化され"
> ([Claude Codeとの会話を、チームの記憶にする ── 開発の文脈をメモリに残して共有する仕組み](https://techblog.zozo.com/entry/memory-manager-context-reuse), セクション本文) ※2026-09-10に実際にfetch成功

> "メモリはなんでも残して検索を強くする"
> ([Claude Codeとの会話を、チームの記憶にする ── 開発の文脈をメモリに残して共有する仕組み](https://techblog.zozo.com/entry/memory-manager-context-reuse), セクション本文) ※2026-09-10に実際にfetch成功

**出典**:
- [Claude Codeとの会話を、チームの記憶にする ── 開発の文脈をメモリに残して共有する仕組み](https://techblog.zozo.com/entry/memory-manager-context-reuse) (ZOZO Tech Blog、Git管理Markdown+SQLite二重索引による組織横断メモリ共有の事例) ※2026-09-10 fetch
