...
#### 追加根拠 (2026-05-16) — ルール5「Claude Code の Skills 機能でチーム作業ワークフローを自動化する」

新たに以下の記事で Skills を個人の学習ループ（ふりかえり自動化）に適用した事例が示された:
- [使うほどふりかえりの深掘りが上達していく Claude Code Skill を作った話](https://zenn.dev/sonicgarden/articles/03c81ca6413ad4) (Zenn sonicgarden / 2026-05-15) ※2026-05-16に実際にfetch成功
...
Skills は「ブランチ作成→PR」などの**チームワークフロー**だけでなく、**個人の学習ループ**としても機能する。
`/daily`（日報管理）と `/furikaeri-kpt`（KPT ふりかえり）の組み合わせにより、
過去のふりかえりパターンを Claude が蓄積・参照し、深掘りの精度が繰り返すたびに向上する。
この「フィードバックを Skill に蓄積し次回に活かす」設計パターンは、
チームの暗黙知を形式知に変える仕組みとしても適用できる。

**確信度**: 既存（高）→ 高（個人学習ループへの適用事例で活用範囲が拡大）
...
#### 追加根拠 (2026-05-16) — ルール10「Git Worktree で Claude Code タスクを並列実行し、開発速度を向上させる」

新たに以下の記事で、ワークツリー並列実行の**反例・落とし穴**が実体験から報告された:
- [【初心者】ワークツリーを切ってイキりまくり、エージェント並行開発をしていたらコンフリクトを連発した話と対策。](https://zenn.dev/wahe/articles/eda7fb1a7a2848) (Zenn wahe / 2026-05-15) ※2026-05-16に実際にfetch成功

**出典引用**:
> 「ブランチは切った時点の状態を元に作業するので、別ブランチにしてワークツリーを立てたとて同じファイルを別々に編集すれば当然衝突する。」
> (セクション "原因：ワークツリーを分けても、同じファイルを触っている")

> 「『他ファイル編集禁止』くらいまで指定した方が安全そうである。」
> (セクション "対策1：編集境界をガチガチに決める")

**追加の知見（反例・例外条件）**:

ワークツリーはブランチを分けても**共通ファイルの競合を自動解決しない**。
特に `_common.scss`・`style.scss`・`main.scss` などの共通スタイルファイルは複数エージェントが同時編集しやすく、コンフリクトが集中する。

**対策の優先順位**（上から試す）:
1. **エージェントごとに編集ファイルを厳格に固定する**: 「エージェントAはこのファイルのみ編集」を明示。共通ファイルは必ずシリアル処理にする
2. **コンポーネント単位の閉じた構造を採用する**: Vue SFC や CSS Modules など「1ファイルに HTML/JS/CSS が閉じている構造」はコンフリクトが起きにくい
3. **並列作業をやめる**: 認知負荷（コンフリクト対応）を考えると「順次実行+人間は要件定義・交通整理に注力」の方が結果的に速いケースがある

**確信度**: 既存（中）→ 中（共通ファイルの競合という反例を追加、編集境界の厳格化を強調）
