...
**コード例（MCP allowedTools 設定）**:
```json
// .claude/settings.json — MCP サーバーの許可ツールを絞る
{
  "mcpServers": {
    "accounting-mcp": {
      "command": "npx",
      "args": ["accounting-mcp"],
      "allowedTools": ["list_account_items", "create_deal"]
    }
  }
}
```
...
- [ ] MCP サーバーを有効にする際は `allowedTools` で必要なツールのみを許可リスト化する（接続＝全ツール全開放がデフォルト）。270個→12個に絞ると不要ツール定義の読み込みによるトークン浪費も消える
...
- [MCPサーバーの権限を絞ったら、AIが触れる範囲が初めて見えた](https://zenn.dev/kenimo49/articles/mcp-permission-scope-minimization-design) (Zenn kenimo49、allowedTools による MCP 内ツール単位の最小権限・Read/Write 分離・マトリクス設計) ※2026-06-20 fetch
