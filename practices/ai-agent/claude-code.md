...
### 21. ネスト型サブエージェントは `.claude/agents/` フロントマターで定義し、階層ごとに Haiku / Sonnet / Opus を使い分ける
...
### 22. マルチエージェント組織は Playbook 孤立と CLAUDE.md 増殖を定期 grep 検査で能動的に検出する
...
### 23. AI エージェントの PR を「意思決定ログ」として Issue 仕様・検証ログ・不採用案の 3 点セットでレビューする
