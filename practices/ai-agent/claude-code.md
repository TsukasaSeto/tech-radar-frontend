...
- `memory file` の「注意書き」だけでは同じ失敗を繰り返す。強制可能な規律は hooks でコードに落とす
...
- **SKILL.md は 500 行以内**に収める。Claude は長い SKILL.md を完全には遵守しない傾向があるため、500 行を超えたら詳細を `references/` に委譲してプログレッシブディスクロージャーを実現する
- **複数フェーズの Skill はオーケストレーター Skill がサブ Skill を呼び出す構造**にする（例: `execute-task` → `execute-coding`, `execute-release`, `execute-deploy`）。1 Skill に全処理を詰め込むと保守性が下がり、500 行制限にも抵触する
- **自動化が怖い箇所には必ず人間確認ポイントを組み込む**。「AI 自動で流れすぎて怖い」という感覚が出た時点で、フェーズ境界（設計確認・実装後テスト・PR 作成前）に強制確認ステップを入れる
- **チームの口頭ルールを Skill にエンコードする**。振り返りで出た改善をそのまま SKILL.md に反映すれば、次回から全員が自動的にルールを守る形になる
...
> "最初の明文化と形になるまでの修正はしんどい。でも1回書いてしまえば、品質が良いものをすごい速さで出し続けられる"
> ([開発フローをClaude Codeスキルにしたら、チーム全員のカレーが毎回おいしくなった](https://zenn.dev/spectee/articles/execute-task-skill), セクション "設計原則") ※2026-05-28に実際にfetch成功

> "一撃必殺プロンプトは、魔法の一文ではありません。短い依頼の裏側に、必要な判断材料がすでにまとまっている状態です。"
> ([Agent Skills で AI への依頼を設計する](https://zenn.dev/genda_jp/articles/2026-05-28-agent-skills-ai-interaction), セクション "Core Concept") ※2026-05-28に実際にfetch成功
...
### 17. Claude を使うワークフローはタスク複雑度に応じてモデルを使い分け、コスト・速度・品質を最適化する

プレビュー生成・ファイルバリデーション・単純な変換など繰り返し発生する軽量タスクには Haiku などの小型モデルを、
アーキテクチャ判断・複雑なデバッグ・大規模リファクタリングには Opus を使う。
単一の高性能モデルで全タスクをこなすとコストが不必要に高騰し、レスポンスも遅くなる。

**根拠**:
- プレビューデプロイやファイルバリデーションは合否判定が主で、高い推論能力を必要としない
- 複雑な実装タスクでは Opus の長いコンテキスト理解・高い推論能力が精度と品質を上げる
- Sentry がマーケティングサイト刷新で Claude Skills に Haiku（プレビュー用）と Opus（複雑な開発用）を使い分けた結果、Web Vitals スコアが 89 → 97 に改善し、ビルド時間が 14分 → 4分未満に短縮された

**コード例**:
```typescript
// Claude API を直接呼ぶ場合のモデル選択例
const client = new Anthropic();

// 軽量タスク（プレビュー確認・ファイルバリデーション）
const previewResponse = await client.messages.create({
  model: "claude-haiku-4-5-20251001",
  max_tokens: 1024,
  messages: [{ role: "user", content: "このHTMLのバリデーションをしてください: ..." }],
});

// 複雑タスク（アーキテクチャ設計・複雑なリファクタリング）
const designResponse = await client.messages.create({
  model: "claude-opus-4-8",
  max_tokens: 8096,
  messages: [{ role: "user", content: "この認証フローのアーキテクチャを設計してください..." }],
});
```
...
| タスク種別 | 推奨モデル | 理由 |
|---|---|---|
| プレビュー生成・確認 | Haiku | 高速・低コスト、合否判定が主 |
| ファイルバリデーション | Haiku | パターンマッチングが主 |
| コンテンツ更新ガイド | Haiku | 定型フォーマットへの誘導 |
| 複雑なリファクタリング | Opus | 長いコンテキスト・高推論能力が必要 |
| アーキテクチャ設計 | Opus | トレードオフ分析・複雑な判断が必要 |
| バグ調査・根本原因分析 | Opus | コードベース横断の推論が必要 |

**出典引用**:
> "The team uses lighter models (Haiku) for preview tasks while reserving Opus 4.6 for complex development work."
> ([How we cut build times by two-thirds by deleting our CMS](https://blog.sentry.io/cut-build-times-delete-cms/), セクション "Model selection") ※2026-05-28に実際にfetch成功

**バージョン**: Claude API / Claude Code（全バージョン）
**確信度**: 中
**最終更新**: 2026-05-28
