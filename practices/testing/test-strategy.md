...
### 8. エージェントテストをテストピラミッドの第4層として位置づけ、決定論的テストと組み合わせる

AI エージェントを E2E テスト実行者として活用する「エージェントテスト」は、従来の決定論的テスト（特定のジャーニーを強制）とは根本的に異なる。
エージェントは「ジャーニーではなくゴールを検証」し、UI の変化に 80% 以上のケースで適応しながら同じ結果に到達できる。
高頻度 CI には不向き（$15〜30/実行、5〜11 分）だが、フレーキーなワークフローのデバッグや複雑な UI の探索的検証に特化した層として組み合わせる。

**根拠**:
- "Tests enforce journeys. Agents verify goals." — 従来テストは手順を強制し、エージェントは目標を検証する。どちらかを廃止するのではなく補完させる
- Slack の実験では、エージェントは 80% 以上のケースで経路を変えながらも同一の結果に到達し、決定論的テストでは検出できない UI 柔軟性の問題を発見できた
- Agent+MCP 構成の失敗率は 0〜12%（CLI ベースは 12〜20%）、MCP 経由のブラウザ制御は消費ターン数が少ない（約 40〜60 ターン vs 85 ターン）
- コスト（$15〜30/実行）と時間（5〜11 分）の問題から高頻度 CI には向かない。週次 or 変更トリガーで選択的に実行する

**コード例**:
```
テスト戦略の層構成（エージェントテスト追加後）:

従来（ピラミッド/ダイヤモンド）:      拡張後:
      /E2E\                                /E2E\        ← 重要フロー決定論的テスト（CI常駐）
     /------\                             /-------\     ← エージェントテスト（週次/定期）
    / Integ. \                           / Integ.  \
   /----------\                         /----------\
...
エージェントテストが有効なシナリオ:
- フレーキーテストの根本原因調査（経路を変えても同じ結果になるか確認）
- 複雑な UI フローの探索（ユーザーが実際にどう操作するかの発見）
- 本番バグの再現（厳密な手順が不明なバグの調査）

導入コスト目安（Slack 実験値）:
  1回の実行: $15〜30 / 実行時間: 5〜11 分
  → 高頻度 CI には組み込まず、週次または障害調査トリガーで選択的実行
```

**アンチパターン**:
- エージェントテストを高頻度 CI に組み込む（コスト・時間の問題で持続不可能）
- 決定論的 E2E テストをエージェントテストで置き換える（安定した CI 基盤が失われる）

**出典引用**:
> "The most effective testing strategies of the future will combine both. Deterministic tests provide a stable foundation for CI, while agentic testing adds a distinct layer."
> ([Agentic Testing: Where Agents Fit in the E2E Testing Stack](https://slack.engineering/agentic-testing-where-agents-fit-in-the-e2e-testing-stack/), セクション "Where Agentic Testing Fits in the Testing Pyramid") ※2026-06-11に実際にfetch成功

**バージョン**: Playwright MCP + 各 LLM API（LLM 依存のため固定なし）
**確信度**: 高
**最終更新**: 2026-06-11

---

### 9. AI生成テストの「ミラー比率」を静的監査し、グリーンCIが隠す検証空白を検出する

同一エージェントが実装とテストを同時に生成した場合、テストが実装を「鏡写し」にするだけの形になりやすい。
AST 解析でテストコードのパターンを静的に検出し、ミラー比率（独立した検証信号を持たないテストの割合）を CI ゲートに組み込む。
...
**ミラーパターンの4種類**:
```python
# 1. 再計算テスト（f(x) == f(x）— 式が実装式を再現
assert compute_tax(100, 0.1) == compute_tax(100, 0.1)

# 2. ゴールデンリテラル — 前回実行結果をハードコード
assert result == 42  # 実装から拾った値がそのまま期待値に

# 3. スモークテスト — 意味ある assertion がない
def test_init(): obj = MyClass(); assert obj is not None

# 4. self-grading マーカー — 生成マークで低信頼を示唆
# generated by Claude Code  # 人間レビューが省略されている兆候
```

```bash
# CI ゲート: ミラー比率 30% 超でビルドブロック
python mirror_audit.py tests/ --max-mirror-ratio 0.30 || {
  echo "Mirror ratio exceeded threshold. Review and add independent assertions."
  exit 1
}
```

**採用判断軸**:
- 「この機能を削除しても、このテストは失敗するか？」→ No ならミラーテスト候補
- ネガティブケース・境界値・エラーパスのテストは独立した検証信号を持ちやすい
- AI 生成テストを取り込む前に、必ず人間が assertion の根拠（なぜその期待値か）を確認する

**アンチパターン**:
- AI にテストを生成させた後、内容を確認せずにそのまま commit する
- カバレッジ % のみを品質指標とする（ミラーテストはカバレッジを上げながら品質を担保しない）
- エラーパスのテストを省略する（ミラーテストはすべて正常系にかたよりやすい）

**出典引用**:
> "mirror-ratio measures MISSING INDEPENDENT SIGNAL, not bug-rate"
> ([Audit AI-Generated Tests: Half of Green CI Proves Nothing](https://dev.to/alex_spinov/audit-ai-generated-tests-half-of-green-ci-proves-nothing-4bmb), セクション "Core Insight: Missing Independent Signal") ※2026-06-22に実際にfetch成功
...
### 10. プロパティベース／不変条件（invariant）テストで、個別ユニットテストが見逃す「組み合わせ」の不具合を検出する

個々の関数が単体では正しくても、複数の正しい関数を組み合わせた結果、意図しない非対称な挙動が生まれることがある。「押せば必ず動く」「増減は対称」「往復すると元に戻る」といった不変条件（invariant / property）を明示してテストすることで、特定の入力値の組み合わせでしか顕在化しない不具合を検出できる。

**根拠**:
- 個別の値でのユニットテストは「その値では正しい」ことしか保証しない。境界値の丸め処理などは入力値の組み合わせ次第で非対称な挙動を生みやすい
- 「どのコンポーネントが挙動を決定する責任を持つか」を明確化しないまま複数の正しい関数を組み合わせると、責任の所在があいまいな箇所（丸め処理など）が意図せず他のロジックを支配してしまう
- 実例では `Math.round` が常に切り上げ方向に丸めるため増減ボタンの挙動が非対称になっていたが、個別の値をアサートするユニットテストは21本すべてグリーンのまま検出できず、手動テストで発覚した

**コード例**:
```ts
// Bad: 個別の入力値だけをアサートする（21本グリーンでも非対称バグを見逃す）
test('90kgから+2.5kgすると92.5kgになる', () => {
  expect(increment(90, 2.5)).toBe(92.5);
});
test('92.5kgから-2.5kgすると90kgになる', () => {
  expect(decrement(92.5, 2.5)).toBe(90);
});

// Good: 「往復すると元に戻る」という不変条件をプロパティとして検証する
import fc from 'fast-check';

test('increment/decrement は可逆（round-trip）である', () => {
  fc.assert(
    fc.property(fc.float({ min: 0, max: 200 }), (weight) => {
      const step = getWeightStep(availablePlates);
      expect(decrement(increment(weight, step), step)).toBeCloseTo(weight);
    })
  );
});

// Good: 丸め処理に依存せず、ステップ幅の倍数で増減する実装に寄せる
function increment(value: number, step: number) {
  return value + step; // Math.round に増減量の決定を委ねない
}
```

**アンチパターン**:
- 個別の入力値のテストだけを増やし、「対称性」「可逆性」「単調性」などの不変条件を明示的に検証しない
- 丸め処理・端数処理のような「誰が挙動を決定する責任を持つか」があいまいな箇所を、テストなしで既存関数の副作用に委ねる

**出典引用**:
> 「押せば必ず動く」「±が対称」「往復すると元に戻る」という性質です
> ([テストが21本すべて通るのに、機能が壊れていた](https://zenn.dev/matsutake_prgrm/articles/unit-tests-pass-but-broken), セクション 不変条件の説明) ※2026-08-07に実際にfetch成功

**出典**:
- [テストが21本すべて通るのに、機能が壊れていた](https://zenn.dev/matsutake_prgrm/articles/unit-tests-pass-but-broken) (Zenn) ※2026-08-07に実際にfetch成功

**バージョン**: フレームワーク非依存（テスト設計原則）
**確信度**: 低（1記事の主張、パターン1c：単独ソース）
**最終更新**: 2026-08-07
