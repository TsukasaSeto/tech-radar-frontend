...
### 5. TypeScript 7.0 RC への段階的移行で CI 型チェックを5〜21倍高速化する

TypeScript 7.0 RC は Go 言語による書き直しにより、従来の `tsc` より5〜21倍高速な型チェックを実現する。
CLI だけ先行移行し、プログラマティック API は TypeScript 7.1 安定版まで v6 に留めることで、リスクを最小化しながら CI の fast feedback を段階的に得る。

**根拠**:
- Go 言語の goroutine による軽量並列処理が型チェックを根本的に高速化する（`tsc --noEmit` 19秒 → 0.9秒の実測例）
- `--checkers` と `--builders` の並列度は **OS・CPU 環境依存** — Linux CI では `--builders 4 --checkers 2` が最速だが、Windows では `--builders 4` のみが最適（`--checkers` 追加でかえって遅化する）
- Vitest / ts-jest / ts-node など TypeScript のプログラマティック API に依存するツールは TypeScript 7.1 安定版まで v6 のまま維持し、npm alias で共存させる
- `--builders 8` は多くの環境で undeclared build dependency エラーを引き起こすため、実測値なしで設定しない

**移行シーケンス**:
```bash
# Step 1: TypeScript 6 互換性確認（paths, rootDir, ES5 targets を先に解消）
npx tsc --noEmit  # エラーゼロを確認

# Step 2: CLI のみ TypeScript 7.0 RC に移行（save-exact でカスケードを防止）
npm install typescript@7.0.1-rc --save-exact --save-dev

# Step 3: 型確認のみ実行
npx tsc --noEmit

# Step 4: プログラマティック API に依存するツールは v6 alias で継続利用
npm install @typescript/typescript6@npm:typescript@^6 --save-dev
# → package.json の scripts で tsconfig.json の paths 依存ツールだけ v6 を使う
```

```yaml
# GitHub Actions — OS 別パラメータの使い分け（実測値なしで変更しない）
jobs:
  typecheck:
    runs-on: ubuntu-latest
    steps:
      - run: npx tsc --noEmit --builders 4 --checkers 2
      # Windows runner の場合: --builders 4 のみ（--checkers 追加で速度低下）
```

**アンチパターン**:
- CSS `@import` などの side-effect import をそのまま移行する（`src/types/css.d.ts` で型宣言を先に用意する）
- `.js + JSDoc` ファイルを速度最適化と同時に扱う（別フェーズで対応）
- プログラマティック API ツールも同時に v7 へ移行しようとする（7.1 安定版まで待つ）

**出典引用**:
> "並列度は環境依存です"
> ([TypeScript 7.0 RCは速いが、見るべきはtsc移行とCIの並列度だった](https://zenn.dev/53able/articles/fa3d26db6f9fba), セクション "CI 別パラメータ調査") ※2026-06-22に実際にfetch成功

> "約21倍の高速化を実測しました"
> ([TypeScript 7.0を実プロジェクトで検証：型チェックが高速化](https://zenn.dev/nenene01/articles/typescript-7-benchmark-musclelog), セクション "ベンチマーク結果") ※2026-06-22に実際にfetch成功

**バージョン**: TypeScript 7.0 RC+（プログラマティック API は 7.1 安定版以降）
**確信度**: 高
**最終更新**: 2026-06-22
