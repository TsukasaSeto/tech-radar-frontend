...
SWRの記事では「手動での状態管理による煩わしさを減らし、可読性や開発效率を高める」ための `useSWR` フックの使用が推奨されている。
`useSWR('/api/user', fetcher)` の一行でデータ取得・エラーハンドリング・ローディング状態管理が実現され、`useState` + `useEffect` による手動実装が不要になることを実証。
POST等の明示的な送信には `useSWRMutation` を使い `isMutating` フラグで二重送信を防止するパターンも紹介されている。
